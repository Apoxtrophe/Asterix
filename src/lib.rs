...
impl Network {
  pub fn new(
    ...
    network.push(Matrix::zeros(network_layout[1], network_layout[0])); //Input -> First hidden weights
    network.push(Matrix::zeros(network_layout[1], 1)); //Bias vector, first hidden layer
    for i in 1..network_layout.len() - 2{
      network.push(Matrix::zeros(network_layout[i + 1], network_layout[i]));
      network.push(Matrix::zeros(network_layout[i + 1], 1));
    }
    network.push(Matrix::zeros(network_layout[length], network_layout[length - 1]));
    network.push(Matrix::zeros(network_layout[length], 1));
    network.push(Matrix::zeros(network_layout[length], 1));
    Network {
      network_layout,
      cluster: network,
